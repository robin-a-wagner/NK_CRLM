...
Visualise effect of library normalisation. 

```{r}

lcpm_norm <- data.frame(edgeR::cpm(expr_norm, log = T))
lcpm_norm <- lcpm_norm %>%
  mutate(gene = rownames(lcpm_norm)) %>%
  tidyr::gather(key = sample, 
         value = expr, 
         -gene); head(lcpm_norm)

lcpm_filt_boxplot <- ggplot(lcpm_filt, aes(x = sample, y = expr, fill = sample)) + 
  geom_boxplot() +
  guides(fill = "none") +
  theme_bw() +
  theme(text = element_text(size = 12),
  ...
  ggtitle("Pre-TMM Normalisation") +
  labs(x = "expr (logCPM)")

lcpm_norm_boxplot <- ggplot(lcpm_norm, aes(x = sample, y = expr, fill = sample)) + 
  geom_boxplot() +
  guides(fill = "none") +
  theme_bw() +
  theme(text = element_text(size = 12),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()) + 
  ggtitle("Post-TMM Normalisation") +
  labs(x = "expr (logCPM)")

ggarrange(lcpm_filt_boxplot, lcpm_norm_boxplot)
